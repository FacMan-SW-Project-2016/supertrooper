...
function setDynamicOptions(selector, keyword, textfield, options) {
	...
	for (key in options) {
		...
		if (html != undefined) {
			...
			html += '<div class="item" data-value="' + options[key][keyword]
					+ '">' + options[key][textfield] + '</div>';
		} else {
			html = '<div class="item" data-value="' + options[key][keyword]
					+ '">' + options[key][textfield] + '</div>';
		}
...
$('#table').on(
		...
		function(e, row, $element) {
			...
			if (row.data !== null) {
				$('#popupPic').attr('src',
						'http://localhost:8000/photo/download/' + row.ID);
			}
			...
			$.ajax({
				...
				success : function(data) {
					setDynamicOptions('#dropdownBuild', 'ID', 'name',
							data.results);
				},
				async : false
			});
			...
			$('#dropdownBuild').dropdown(
					{
						onChange : function(value) {
							...
							$('#dropdownRoom').dropdown({
								showOnFocus : false
							});
							$.ajax({
								url : 'http://localhost:8000/room_building/'
										+ roomofbuilding,
								cache : false,
								type : 'GET',
								success : function(data) {
									...
									setDynamicOptions('#dropdownRoom', 'ID',
											'name', data.results);
			...
			$.ajax({
				...
				type : 'GET',
				success : function(data) {
					setDynamicOptions('#dropdownRoom', 'ID', 'name',
							data.results);
				},
				async : false
			});
			...
			$.ajax({
				...
				success : function(data) {
					...
					setDynamicOptions('#dropdownCat', 'ID', 'text',
							data.results);
				},
				async : false
			});

			$('#dropdownStat').dropdown({
				showOnFocus : false
			});
			...
			$.ajax({
				...
				success : function(data) {
					...
					setDynamicOptions('#dropdownFacMan', 'name', 'name',
							data.results);
				},
				async : false
			});
